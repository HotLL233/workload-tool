...
const StatsPage: React.FC = () => {
  ...
  const hdu = async () => {
    setDul(true);
    try {
      const r = await deleteRdRecordsByUser(dun, { start: si, end: ei });
      if (r.code === 0) {
        setDuo(false);
        lul(ulp);
  ...
  const hsr = async () => {
    if (!sdr) return;
    try {
      const r = await deleteRdRecord(sdr.id);
      if (r.code === 0) {
        setSdr(null);
        lul(ulp);
      } else {
            ...
            >
              <Button
                ...
                sx={{
                  ...
                  "&:hover": {
                    background: "linear-gradient(135deg, #bf4d00, #e65100)",
                  },
                }}
              >
                导出 Excel
