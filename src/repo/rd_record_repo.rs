...
pub fn list(
    ...
    let count: i64 = conn.query_row(
        &format!("SELECT COUNT(DISTINCT wr.id) FROM rd_work_records wr JOIN projects p ON wr.project_id = p.id {}", where_sql),
        rusqlite::params_from_iter(params.iter().map(|p| p.as_ref())),
        |r| r.get(0),
    )?;
...
pub fn create(pool: &DbPool, body: &RecordCreate, batch_no: Option<String>, notes: Option<String>) -> Result<RdRecordResponse> {
    ...
    audit_repo::log_on_conn_with_module(&tx, "create", "rd_work_records", Some(id), &body.user_name, &detail, "rd")?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}

pub fn update(pool: &DbPool, id: i64, body: &RecordUpdate, user_name: &str) -> Result<RdRecordResponse> {
    let mut conn = pool.get()?;
    let tx = conn.transaction()?;
    ...
    if let Some(ref un) = body.user_name {
        ...
        let rows = tx.execute("UPDATE rd_work_records SET user_name=?1 WHERE id=?2", (un, id))?;
        if rows == 0 {
    ...
    if let Some(q) = body.quantity {
        ...
        let rows = tx.execute("UPDATE rd_work_records SET quantity=?1 WHERE id=?2", (q, id))?;
        if rows == 0 {
            return Err(crate::error::AppError::NotFound("记录不存在".into()));
        }
        updated = true;
    ...
    audit_repo::log_on_conn_with_module(&tx, "update", "rd_work_records", Some(id), user_name, &detail, "rd")?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn soft_delete(pool: &DbPool, id: i64, user_name: &str) -> Result<()> {
    ...
    let deleted: Option<String> = tx.query_row(
        "SELECT deleted_at FROM rd_work_records WHERE id=?1", [id], |r| r.get(0)
    ).map_err(|e| match e {
        rusqlite::Error::QueryReturnedNoRows => crate::error::AppError::NotFound("记录不存在".into()),
        _ => e.into(),
    })?;
    if deleted.is_some() { return Err(crate::error::AppError::Validation("记录已被删除".into())); }
    let rows = tx.execute("UPDATE rd_work_records SET deleted_at=datetime('now','localtime') WHERE id=?1", [id])?;
    if rows == 0 {
        return Err(crate::error::AppError::NotFound("记录不存在".into()));
    }
    ...
    audit_repo::log_on_conn_with_module(&tx, "delete", "rd_work_records", Some(id), user_name, &detail, "rd")?;
    tx.commit()?;
    Ok(())
}

pub fn restore(pool: &DbPool, id: i64, user_name: &str) -> Result<RdRecordResponse> {
    let mut conn = pool.get()?;
    let tx = conn.transaction()?;
    ...
    let rows = tx.execute("UPDATE rd_work_records SET deleted_at=NULL WHERE id=?1", [id])?;
    if rows == 0 {
        return Err(crate::error::AppError::NotFound("记录不存在".into()));
    }
    ...
    audit_repo::log_on_conn_with_module(&tx, "restore", "rd_work_records", Some(id), user_name, &detail, "rd")?;
    tx.commit()?;
    get_by_id_on_conn(&conn, id)
}
...
pub fn delete_by_user(pool: &DbPool, user_name: &str, start: Option<&str>, end: Option<&str>) -> Result<i64> {
    let mut conn = pool.get()?;
    let tx = conn.transaction()?;
    let mut sql = "UPDATE rd_work_records SET deleted_at=datetime('now','localtime') WHERE user_name=?1 AND deleted_at IS NULL".to_string();
    ...
    audit_repo::log_on_conn_with_module(&tx, "batch_delete", "rd_work_records", None, user_name, &format!("批量删除 {} 条", count), "rd")?;
    tx.commit()?;
    Ok(count as i64)
}
